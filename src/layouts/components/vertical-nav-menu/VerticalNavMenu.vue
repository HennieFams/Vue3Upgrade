      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <div class="header-sidebar flex items-end justify-between" slot="header">
          ...
          <router-link tag="div" class="vx-logo cursor-pointer flex items-center" to="/">
            ...
            <img src="../../../assets/images/logo/fams.png" style="width: 42px;" class="mr-2 ml-0" />
        ...
        <component :is="scrollbarTag" ref="verticalNavMenuPs" class="scroll-area-v-nav-menu pt-2" :settings="settings" @ps-scroll-y="psSectionScroll" @scroll="psSectionScroll" :key="$vs.rtl">
          <template v-for="(item, index) in menuItemsUpdated">
            ...
            <template v-else-if="!item.header">
              ...
              <!-- Nav-Group -->
              <template v-else-if="item.submenu.filter(x=>$acl.check($router.match(x.slug !== 'external' ? x.url : null).meta.rule)).length>0">
                <v-nav-menu-group
                  :key="`group-${index}`"
