<template>
  <div>
		<customReport-sidebar
      :isSidebarActive="isCustomReportSidebarActive"
...
import CustomReportSidebar from './CustomReportSidebar.vue';
  ...
  data(){
    return{
      ...
      isMounted: false,
			...
			isListHidden: false,   
      isInfoHidden: true,
      isCustomReportSidebarActive: false,
      sidebarData: {},
    ...
    totalRows(rowCount){
      ...
      self.$ajaxGet(self, "CustomReport", onSuccess, onFinally);
		},
    ...
    toggleDataSidebar(val){
        if (val)
        this.isCustomReportSidebarActive = true;
        else{
          this.isCustomReportSidebarActive = false;
          this.getReports();
        }
