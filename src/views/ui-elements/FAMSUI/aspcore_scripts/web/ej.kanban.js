        ...
        if (kObj._currentJsonData.length == 0) {
            if (kObj.model.showColumnWhenEmpty && kObj.model.editSettings.allowAdding) {
                ...
                if (editItems.length == 0)
                    kObj._currentData[kObj.model.fields.primaryKey] = "";
        ...
        for (i = 0; i < columns.length; i++) {
            ...
            if (isExp) {
                ...
                if (cshrink)
                    for (j = 0; j < kObj._columnRows.length; j++) {
                        colval = kObj._columnRows.eq(j).find("td.e-rowcell").eq(i);
                        if (colval.length > 0 && !colval.hasClass("e-shrink")) {
                            ...
                            var contentTop = colval.offset().top;
...
(function ($) {
    var Kanban = (function (_super) {
        ...
        Kanban.prototype._destroy = function () {
            ...
            this.element.removeClass("e-kanban e-widget e-onlycontent").removeAttr("accesskey");
        ...
        Kanban.prototype._columnCardcount = function (id, key, index, proxy) {
            ...
            else if (id.length > 0 && !ej.isNullOrUndefined(id[index])) {
        ...
        Kanban.prototype._getColumnKeyItems = function (objectId, togglekey, count, cards) {
            ...
            if (togglekey) {
                ...
                for (var i = 0; i < key.length; i++)
                    predicate.push(new ej.Predicate(kfield, ej.FilterOperators.equal, key[i], false));
            cloned = cloned.where(ej.Predicate["or"](predicate));
            if (ds.GROUPGUID)
