<template>
  ...
  >
    ...
    <vs-divider class="mb-3">New Report</vs-divider>
...
export default {
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(self.data).length === 0) {
        ...
        self.formData.reportTypeId = self.data.reportTypeId;
  ...
  computed: {
    ...
    uploadReportLocation: {
      get() {
        return this.$buildUrl(`CustomReport/GetReportString`);
      ...
      self.$validator.validateAll().then((result) => {
        if (result) {
          if(!self.formData.report1){
            ...
            self.$ajaxPost(self, "CustomReport/", self.formData, onSuccess, onFinally);
          }
        }
        else{
