    ...
    ej.Pivot = {
       ...
       addReportItem: function (dataSource, args) {
        ...
        closePreventPanel: function (args) {
            ...
            if (!ej.isNullOrUndefined(ctrlObj.model.pivotControl) && !ej.isNullOrUndefined( ctrlObj.model.pivotControl.element)) {
        ...
        _createErrorDialog: function (args, action, obj) {
            ...
            if (obj.element.find(".e-errorDialog:visible").length == 0) {
                if (!ej.isNullOrUndefined(args.Exception))
                    var dialogElem = ej.buildTag("div.e-errorDialog#" + this._id + "_ErrorDialog", ej.buildTag("div.warningImg")[0].outerHTML + ej.buildTag("div.warningContent action:", (args.Exception.Message))[0].outerHTML + ej.buildTag("br")[0].outerHTML + ej.buildTag("div." + obj._id + "stackTraceContent", "Stack Trace :" + args.Exception.StackTraceString)[0].outerHTML + ej.buildTag("div", ej.buildTag("button#" + this._id + "_ErrOKBtn.errOKBtn", obj._getLocalizedLabels("OK"), { "margin": "20px 0 10px 165px" }).attr("title", obj._getLocalizedLabels("OK").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML)[0].outerHTML).attr("title", action)[0].outerHTML;
                else
                    var dialogElem = ej.buildTag("div.e-errorDialog#" + this._id + "_ErrorDialog", ej.buildTag("div.warningImg")[0].outerHTML + ej.buildTag("div.warningContent action:", (args))[0].outerHTML + ej.buildTag("div", ej.buildTag("button" + this._id + "_ErrOKBtn.errOKBtn", obj._getLocalizedLabels("OK") , { "margin": "20px 0 10px 165px" }).attr("title", obj._getLocalizedLabels("OK").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML)[0].outerHTML).attr("title", action)[0].outerHTML;
                ...
                obj.element.find(".errOKBtn").ejButton({ type: ej.ButtonType.Button, width: "50px" }).on(ej.eventType.click, function (e) {
                    ...
                    if (obj._errorDialog == obj._getLocalizedLabels("Success") || obj._errorDialog == obj._getLocalizedLabels("Warning") || obj._errorDialog == obj._getLocalizedLabels("Exception") || obj._errorDialog == obj._getLocalizedLabels("Error"))
                        obj.element.children("#" + this._id + "_ErrorDialog_wrapper").remove();
                });
       ...
       _contextMenuOpen: function (args, ctrlObj) {
            ...
            if(ctrlObj.pluginName == "ejPivotGrid"){
                ...
                menuObj = $("#" + this._id + "_pivotTree").data('ejMenu');
            }
            else {
            ...
            if (mode == ej.Pivot.AnalysisMode.Olap) {
                ...
                if ($(args.target).hasClass("e-btn")) {
                    ...
                    if ($("#" + this._id + "_pivotTree").length > 0) {
        ...
        _searchTreeNodes: function (args) {
            ...
            if (searchVal.length > 0) {
                ...
                if (treeElement.find("li span.e-searchfilterselection").closest("li").length > 0)
                    treeobj.removeNode(treeElement.find("#" + this._id + "_SearchFilterSelection"));
...
//            ctrlObj._memberTreeObj.model.beforeExpand = ej.proxy(ctrlObj._beforeNodeExpand, ctrlObj);
        ...
        _separateAllMember: function (ctrlObj) {
            ...
            var allMember = !isClientServer ? ej.buildTag("div.e-checkAllBox", ej.buildTag("input#" + ctrlObj._id + "_allelement.allMember", "", { "float": isRTL ? "right" : "left" }, { name: 'row', type: 'checkbox', tabindex: 0 })[0].outerHTML + " " + ej.buildTag("label.e-allElementLabel", (isAdvancedFilter) ? "(Select All)" : "All", { "margin-left": "5px" })[0].outerHTML, { width: "100px", float: isRTL ? "right" : "left" })[0].outerHTML : "";
        ...
        createErrorDialog: function (controlObj) {
            ...
            if (controlObj.element.find(".e-errorDialog").length == 0) {
                var dialogElem = ej.buildTag("div.e-errorDialog#" + this._id + "_ErrorDialog", ej.buildTag("div.warningImg")[0].outerHTML + ej.buildTag("div.warningContent", controlObj._getLocalizedLabels("AlertMsg"))[0].outerHTML + ej.buildTag("div", ej.buildTag("button#" + this._id + "_ErrOKBtn.errOKBtn", "OK")[0].outerHTML)[0].outerHTML).attr("title", controlObj._getLocalizedLabels("Warning"))[0].outerHTML;
        ...
        _createDrillThroughDialog: function (args, dataSourceInfo) {
            ...
            var textTitle = ej.buildTag("label#" + this._id + "_", args._getLocalizedLabels("SelectHierarchy"))[0].outerHTML;
            var textArea = "<br><textarea id='" + this._id + "_hrSel' style='width:270px; height:300px; resize:none; margin:0px 5px 0 5px'></textarea></br><br>",
            ...
            var dialogContent = ej.buildTag("div#" + this._id + "_dropDlg.dropDlg", "<table class=\"e-outerTable\"><tr><td>" + browserPanel + "</td><td>" + textTitle + textArea + "</td></tr></table>")[0].outerHTML + "</br>",
            dialogFooter = ej.buildTag("div", ej.buildTag("button#" + this._id + "_btnOK.dialogBtnOK", args._getLocalizedLabels("OK")).attr("title", args._getLocalizedLabels("OK").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML + ej.buildTag("button#" + this._id + "_btnCancel.dialogBtnCancel", args._getLocalizedLabels("Cancel")).attr("title", args._getLocalizedLabels("OK").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML, { "float": "right", "margin": "-5px 0 6px" })[0].outerHTML,
            ejDialog = ej.buildTag("div#" + this._id + "_clientDialog.e-clientDialog", dialogContent + dialogFooter, { "opacity": "1" }).attr("title", "Hierarchy Selector")[0].outerHTML;
            $(ejDialog).appendTo("#" + args._id);
            $("#" + this._id + "_btnOK, #" + this._id + "_btnCancel").ejButton();
            $("#" + this._id + "_btnOK, #" + this._id + "_btnCancel").css({ margin: "0 20px 20px 0", width: "50px" });
            ...
            var grid = args;
            $("#" + this._id + "_btnOK").click(function () {
                var text = $("#" + this._id + "_hrSel").val();
        ...
        _hierarchyNodeDropped: function (sender) {
            if (sender.dropTarget[0].id == "hrSel") {
                ...
                for (var i = 0; i < $("#" + this._id + "_hrSel").val().split(",").length; i++) {
                    if (target == $("#" + this._id + "_hrSel").val().split(",")[i])
                        return false;
                }
                var tex = this.element.find('#" + this._id + "_hrSel').val();
                if (tex.length != 0) {
                    tex = tex + ',' + target;
                    this.element.find('#" + this._id + "_hrSel').val(tex);
                }
                else
                    this.element.find('#" + this._id + "_hrSel').val(target);
            }
            }
        },
                ...
                if (controlObj.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
                    ...
                    if (excludeMem.length > 0 && controlObj._fieldMembers[item.DimensionName])
                        ...
                        for (var i = 0; i < controlObj._clientReportCollection.length; i++) {
                            if (!ej.isNullOrUndefined(controlObj._clientReportCollection[i]._fieldSelectedMembers) && ((controlObj._currentRecordName == "" && controlObj.element.find("#" + this._id + "_reportList") != undefined && controlObj.element.find("#" + this._id + "_reportList")[0].value == controlObj._clientReportCollection[i].name ? true : false) || controlObj._currentRecordName != "" && controlObj._currentReportName == controlObj._clientReportCollection[i].name)) {
        ...
        createAdvanceFilterTag: function (args, ctrlObj) {
            ...
            if (args.action == "filterTag") {
                ...
                filterTag = ej.buildTag("div#" + this._id + "_sortDiv.sortDiv",
                            ej.buildTag("li#" + this._id + "_ascOrder.e-ascOrder", ej.buildTag("span.e-ascImage").addClass("e-icon").attr("aria-label", "ascending")[0].outerHTML + ctrlObj._getLocalizedLabels("Sort") + " A to Z")[0].outerHTML +
                            ej.buildTag("li#" + this._id + "_descOrder.e-descOrder", ej.buildTag("span.e-descImage").addClass("e-icon").attr("aria-label", "descending")[0].outerHTML + ctrlObj._getLocalizedLabels("Sort") + " Z to A")[0].outerHTML)[0].outerHTML;
            }
                ...
                if (args.action == "labelFilterDlg") {
                    dialogContent = ej.buildTag("table.labelfilter",
                                            ...
                                            ej.buildTag("tr", ej.buildTag("td", "<input type='text' id='" + this._id + "_filterOptions' class='filterOptions'  style='width:220px'/>")[0].outerHTML +
                                            ej.buildTag("td.filterValuesTd", "<input type='text' id='" + this._id + "_filterValue1'  value='" + (args.filterInfo.length > 0 ? (opMode == ej.Pivot.OperationalMode.ClientMode ? args.filterInfo[0].values[0] : args.filterInfo[0].value1) : "") + "' style='display:inline; width:160px; height:19px; margin-left:7px;' class='e-filterValues'/></br>")[0].outerHTML)[0].outerHTML)[0].outerHTML;
                    ...
                    innerContent = !($(args["selectedArgs"].element).hasClass("topCount")) ?
                                     (ej.buildTag("td", "<input type='text' id='" + this._id + "_filterMeasures' class='filterMeasures' />").attr("width", "180px")[0].outerHTML +
                                     ej.buildTag("td", "<input type='text' id='" + this._id + "_filterOptions' class='filterOptions'/>").attr("width", "180px")[0].outerHTML +
                                     ej.buildTag("td.filterValuesTd", "<input type='text' id='" + this._id + "_filterValue1' value='" + (args.filterInfo.length > 0 ? (opMode == ej.Pivot.OperationalMode.ClientMode ? args.filterInfo[0].values[0] : args.filterInfo[0].value1) : "") + "' style='display:inline; width:190px; height:19px;' class='e-filterValues'/>" + (colSpan == "4" ? "<span>"+ ctrlObj._getLocalizedLabels("and")+" </span><input type='text' id='" + this._id + "_filterValue2' value='" + (args.filterInfo.length > 0 ? (opMode == ej.Pivot.OperationalMode.ClientMode && !ej.isNullOrUndefined(args.filterInfo[0].values[1])) ? args.filterInfo[0].values[1] : ((opMode == ej.Pivot.OperationalMode.ServerMode && !ej.isNullOrUndefined(args.filterInfo[0].value2)) ? args.filterInfo[0].value2 : "") : "") + "' style='display:inline; width:190px; height:19px;' class='e-filterValues'/> </br>" : "</br>"))[0].outerHTML) :

                                     ej.buildTag("td", "<input type='text' id='" + this._id + "_filterOptions' class='filterOptions' />").attr("width", "80px")[0].outerHTML +
                                     ej.buildTag("td", "<input type='text' id='" + this._id + "_filterValue1' class='e-filterValues' />").attr("width", "50px")[0].outerHTML +
                                     ej.buildTag("td", "<input type='text' id='" + this._id + "_filterMeasures' class='filterMeasures' />").attr("width", "180px")[0].outerHTML;
                ...
                var dialogFooter = ej.buildTag("div", ej.buildTag("button#" + this._id + "_filterDlgOKBtn.e-dialogOKBtn", ctrlObj._getLocalizedLabels("OK")).attr("title", ctrlObj._getLocalizedLabels("OK").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML + ej.buildTag("button#" + this._id + "_filterDlgCancelBtn.e-dialogCancelBtn", ctrlObj._getLocalizedLabels("Cancel")).attr("title", ctrlObj._getLocalizedLabels("Cancel").replace(/(<([^>]+)>)/ig, ""))[0].outerHTML, { "float": ctrlObj.model.enableRTL ? "left" : "right", "margin": "8px 0 6px" })[0].outerHTML;
                ctrlObj.element.find(".e-dialog").remove();
                $(ej.buildTag("div#" + this._id + "_filterDialog.filterDialog", dialogContent + dialogFooter, { "opacity": "1" })).appendTo("#" + ctrlObj._id);
                ...
                if ($(args["selectedArgs"].element).attr("id") == "topCount") {
                    ctrlObj.element.find("#" + this._id + "_filterOptions").ejDropDownList({
                    ...
                    ctrlObj.element.find("#" + this._id + "_filterValue1").ejNumericTextbox({ value: (args.filterInfo.length > 0) ? parseInt(args.filterInfo[0].value1) : 5, minValue: 1 });
                }
                else {
                ...
                ctrlObj.element.find("#" + this._id + "_filterDlgOKBtn").ejButton({
                    type: ej.ButtonType.Button,
                    click: ej.proxy(ctrlObj._filterElementOkBtnClick, ctrlObj)
                });
                ctrlObj.element.find("#" + this._id + "_filterDlgCancelBtn").ejButton({
