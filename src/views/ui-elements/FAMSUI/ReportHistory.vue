<template>
  <div>
    <h3 class="p-3"> Report Scheduled History </h3>
    <vs-tabs style="width: auto min-height:500px">
      <vs-tab label="Report Schedule Per Month" style="min-height:500px">
        <div class="vx-row">
        ...
        <br>
          ...
          <div>
            ...
            >
              <vs-dropdown vs-trigger-click class="cursor-pointer mr-4">
                <div class="p-4 border border-solid border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                  <span class="mr-2" >{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ reportQueueData.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : reportQueueData.length }} of {{ reportQueueData.length }}</span>
            ...
            <template slot="thead">
              ...
              <vs-th sort-key="reportStatus">Report Status</vs-th>
            ...
            <template slot-scope="{ data }">
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td :data="tr.report">{{ tr.report 	}}</vs-td>
                <vs-td :data="tr.emailSend">{{ tr.emailSend.replace("T"," ") 	}}</vs-td>
                <vs-td :data="tr.reportStatus">{{  tr.reportStatus  }}</vs-td>
   ...
   cleanString(string) {
    ...
    GetGrid(){
      ...
      var onSuccess = function (response) {
        ...
        self.reportQueueData = response.data;
      ...
      self.$ajaxGet(
        self,
         `ReportSchedules/GetReportSchedulesHistory?&fromdate=${tempstartDate}
          &toDate=${tempendDate}`,
        onSuccess,
        onFinally
      );
