      ...
      <div>
        ...
        >
          ...
          <template slot="thead">
            ...
            <vs-th sort-key="ShowPopup">Show Popup</vs-th>
            <vs-th>Action</vs-th>
          </template>
          <template slot-scope="{data}">
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <vs-checkbox class="float-left" style="width: fit-content" name="cbPopup" @change.native="checkHomeScreenMessage(tr)" v-model="tr.homeScreenMessage"></vs-checkbox>
              </vs-td>
              <vs-td id="iconContainer" class="whitespace-no-wrap">
                <vx-tooltip class="flex" text="Edit User Information">
                  <feather-icon
                  icon="EditIcon"
                ...
                <vx-tooltip class="flex" text="Delete User">
                  <feather-icon
                  icon="TrashIcon"
...
import UserSidebar from './UserAdminSidebar.vue';
export default {
  data() {
    return {
  ...
  methods: {
   ...
   successNotify() {
    ...
    getUsersList() {
      ...
      self.$ajaxGet(
        self,
         "Users/GetUserListAdmin",                             // + "1",//change that hy vir elke user a role gee role userService line 55
        onSuccess,
        onFinally
      );
    },
    deleteUser(id) {
      ...
      self.$ajaxDelete(
        self,
         `Users/${id}`,//&accountId=${localStorage.getItem('userAccountId')}`,
        onSuccess,
        onFinally
      );
    },
    checkHomeScreenMessage(data) {
      var self = this;
      var obj = {
        Id: data.id,
        Popup: data.homeScreenMessage,
      };
