<template>
  <div>
		<standardReport-sidebar
      :isSidebarActive="isStandardReportSidebarActive"
        ...
        <template slot-scope="{ data }">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td id="iconContainer" class="whitespace-no-wrap">
              <vx-tooltip class="flex" text="Assign Report">
                <feather-icon
                  icon="SaveIcon"
                  svgClasses="w-5 h-5 hover:text-primary stroke-current"
                  @click.stop="AssignReportToAccount(tr)"
                />
              </vx-tooltip>
...
import standardReportSidebar from './StandardReportAdminSidebar.vue';
...
export default {
  data(){
    return{
      ...
      isMounted: false,
			...
			isListHidden: false,   
      isInfoHidden: true,
      isStandardReportSidebarActive: false,
      sidebarData: {},
    ...
    totalRows(rowCount){
      ...
      self.$ajaxGet(self, "ReportStandards/GetReports", onSuccess, onFinally);
		},
  ...
  successNotifyLoadCustom(msg) {
    ...
    AssignReportToAccount(tr)
    {
        var accountName =  localStorage.getItem('userAccount');
        this.selected = tr;
        this.$vs.dialog({
        type:'confirm',
        color: 'primary',
        title: '***** ' + accountName + ' *****' ,
        text: 'Assign :' + tr.name + ' to FAMS Account ' + accountName,
     ...
     acceptAlert(){
        ...
        myobject.ReportStandardId = self.selected.id;
        myobject.Enable = self.selected.enable;
         var onSuccess = function (response) {
            ...
            self.$ajaxPost(self, "PerReportStandardAccounts", myobject, onSuccess, onFinally);
    },
    ...
    toggleDataSidebar(val){
        if (val)
        this.isStandardReportSidebarActive = true;
        else{
          this.isStandardReportSidebarActive = false;
          this.getReports();
        }
