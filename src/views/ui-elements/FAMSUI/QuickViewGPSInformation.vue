<template>
  <div>
    <h3 class="p-3">FAMS' GPS 3rd Party Information Page</h3>
    <vs-tabs style="width: auto min-height:500px">
      <vs-tab label="Axxiss Information" style="min-height: 500px">
        <template>
          <vx-card class="pb-3" refresh-content-action @refresh="closeCardAnimationDemo" collapse-action>
            <div class="vx-row">
              <div class="vx-col w-full mb-base">
                ...
                <div class="vx-col w-full mb-base">
                  <vs-button ref="submit" id="addInfo" class="rounded-lg w-full vs-con-loading__container"
                    color="primary" @click="GetGrid()">Submit</vs-button>
                </div>
              </div>
            </div>
              ...
              <div class="vx-row">
                <div class="vx-col w-full mb-base">
                  <vs-table ref="gridTable" v-model="selected" pagination :max-items="itemsPerPage" search
                    :data="GPSData" class="vs-con-loading__container">
            ...
            formatDate(
              tr.createdate.replace("T", " ").split(".")[0]
                        ...
                        <vs-td class="whitespace-no-wrap" id="iconContainer">
                          ...
                          <vx-tooltip class="flex" text="Latest Axxis GPS Info">
                            <feather-icon icon="EditIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current"
                              class="ml-2" @click.stop="showStatusPrompt(tr)" />
                          </vx-tooltip>
                          <vx-tooltip class="flex" text="Axxis | FAMS Update EQP">
                            <feather-icon icon="SettingsIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current"
                              class="ml-2" @click.stop="showstatusPromptAxxis(tr)" />
                          </vx-tooltip>
                        </vs-td>
                        <template class="expand-user" slot="expand">
                          <div class="con-expand-users w-full">
                            <vs-table ref="gridTable2" v-model="selected" :data="tr.udTrack">
                              <template slot="thead">
                                <vs-th>name</vs-th>
                                <vs-th>uid</vs-th>
                              ...
                              <template slot-scope="{ data }">
                                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                                  ...
                                  <vs-td :data="tr.gpsTimeLocal">{{ tr.gpsTimeLocal }}</vs-td>
                                  <vs-td :data="tr.odometer">{{ tr.odometer }}</vs-td>
                                  <vs-td :data="tr.engineTime">{{ tr.engineTime }}</vs-td>
                                  <vs-td :data="tr.engineStatus">{{ tr.engineStatus }}</vs-td>
                                  <vs-td :data="tr.speed">{{ tr.speed }}</vs-td>
                                  <vs-td :data="tr.speedMeasure">{{ tr.speedMeasure }}</vs-td>
                                  <vs-td :data="tr.address">{{ tr.address }}</vs-td>
                                  <vs-td :data="tr.heading">{{ tr.heading }}</vs-td>
                                  <vs-td :data="tr.ignition">{{ tr.ignition }}</vs-td>
                                  <vs-td :data="tr.latitude">{{ tr.latitude }}</vs-td>
                                  <vs-td :data="tr.longitude">{{ tr.longitude }}</vs-td>

                                  <template class="expand-user" slot="expand">
                                    <div class="con-expand-users w-full">
                                      <vs-table ref="gridTable3" v-model="selected" :data="tr.sensorInfo">
                                        <template slot="thead">
        ...
        <vs-popup id="mapPop" title="Pick Location" fullscreen :active.sync="popupActive">
          <div id="app">
            <GmapMap :center="center" :zoom="5" map-style-id="roadmap" :options="mapOptions"
              style="width: auto; height: 70vmin" ref="mapRef" id="mapID">
              <GmapMarker :position="marker.position" :clickable="true" :draggable="false" @click="panToMarker" />
            </GmapMap>
          </div>
        </vs-popup>
        <vs-popup ref="statusPrompt" id="statusPrompt" title="Last Axxis Information" fullscreen :active.sync="statusPromptActive">
          <div>
            <vs-input v-model="Axxisdata.message" class="w-full" disabled name="Name"></vs-input>
          </div>
    ...
    <vs-prompt ref="statusPromptAxxissUpdate" title="Update Transaction's Axxis Info" @accept="UpdateGPSData"
      acceptText="Yes" cancelText="No" :active.sync="statusPromptAxxis">
...
export default {
  ...
  methods: {
    ...
    GetGrid() {
      ...
      self.$ajaxGet(
        self,
        "Integration3dtracking/Get3DTrackDetailInformation?" + famsURL,
        onSuccess,
        onFinally
      );
    },
