<template>
  ...
  >
    ...
    <vs-divider class="mb-3">System Report Window</vs-divider>
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(self.data).length === 0) {
        ...
        self.formData.report1 = self.data.report1;
      }
    },
  },
  computed: {
    ...
    uploadReportLocation: {
      get() {
        return this.$buildUrl(`StandardSystemReport/GetReportString`);
        }
    },
  },
  methods: {
    ...
    processReportData(error, file, a, b){
      ...
      self.formData.report1 = file.serverId;
    },
    saveReport(){
      var self = this;
        ...
        if (result) {
          if(!self.getReportTypeDropDown){
            ...
            var onFinally = function () {
              ...
              //self.successNotifyLoadCustom(self.$validator.errors.items[0].msg));
            };
            self.$ajaxPost(self, "StandardSystemReport/", self.formData, onSuccess, onFinally);
          }
    ...
    getReportTypeDropDown() {
      ...
      self.$ajaxGet(
        self,
        "StandardSystemReport/GetReportTypeDropDown",
        onSuccess,
        onFinally
      );
    },
